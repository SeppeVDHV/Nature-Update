...
import net.seppevdh.natureupdate.recipe.ShapelessCraftingFurnaceRecipe;
...
public class ShapelessCraftingFurnaceRecipeCategory implements IRecipeCategory<ShapelessCraftingFurnaceRecipe> {
    public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(NatureUpdate.MOD_ID, "shapeless_crafted_cooking");
    ...
    public static final IRecipeType<ShapelessCraftingFurnaceRecipe> SHAPELESS_CRAFTING_FURNACE_RECIPE_TYPE =
            new IRecipeType<>() {
                @Override
                ...
                @Override
                public Class<? extends ShapelessCraftingFurnaceRecipe> getRecipeClass() {
                    return ShapelessCraftingFurnaceRecipe.class;
                }
            };
    ...
    public ShapelessCraftingFurnaceRecipeCategory(IGuiHelper helper, List<ItemStack> fuelItems, List<ItemStack> soulFuelItems, List<ItemStack> endFuelItems) {
    ...
    @Override
    public @NotNull IRecipeType<ShapelessCraftingFurnaceRecipe> getRecipeType() {
        return SHAPELESS_CRAFTING_FURNACE_RECIPE_TYPE;
    }

    @Override
    public @NotNull Component getTitle() {
        return Component.translatable("recipetype.natureupdate.shapeless_crafted_cooking");
    }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, ShapelessCraftingFurnaceRecipe recipe, @NotNull IFocusGroup focuses) {
        for (int i = 0; i < recipe.ingredients().size(); i++) {
            builder.addSlot(RecipeIngredientRole.INPUT, 6 + (i % 3) * 18, 6 + Math.floorDiv(i, 3) * 18).add(recipe.ingredients().get(i));
        }
        for (int i = recipe.ingredients().size(); i < 9; i++) {
            builder.addSlot(RecipeIngredientRole.RENDER_ONLY, 6 + (i % 3) * 18, 6 + Math.floorDiv(i, 3) * 18).add(ItemStack.EMPTY);
        }
    ...
    @Override
    public void draw(ShapelessCraftingFurnaceRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
    ...
    @Override
    public void createRecipeExtras(IRecipeExtrasBuilder builder, ShapelessCraftingFurnaceRecipe recipe, IFocusGroup focuses) {
